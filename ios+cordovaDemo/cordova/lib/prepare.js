...
/**
 * Updates config files in project based on deviceReady's config.xml and config munge,
...
function updateConfigFile (sourceConfig, configMunger, locations) {
    ...
    // Merge changes from deviceReady's config.xml into platform's one
 ...
 *   'platform_www' and deviceReady www. Also copies project's overrides' folder into
...
function updateIcons (cordovaProject, locations) {
    ...
    if (icons.length === 0) {
        events.emit('verbose', 'This deviceReady does not have icons defined');
        return;
    }
...
function updateSplashScreens (cordovaProject, locations) {
    ...
    if (splashScreens.length === 0) {
        events.emit('verbose', 'This deviceReady does not have splash screens defined');
        return;
    }
...
function updateFileResources (cordovaProject, locations) {
    ...
    if (files.length === 0) {
        events.emit('verbose', 'This deviceReady does not have additional resource files defined');
        return;
    }
 ...
 * fall back to the regular launch images that might be supplied (that is, our deviceReady will be scaled on an iPad Pro),
