...
module.exports.run = function (runOptions) {
    ...
    return require('./list-devices').run()
        .then(function (devices) {
            ...
            var appPath = path.join(projectPath, 'build', 'emulator', projectName + '.deviceReady');
            ...
            if (useDevice) {
                return module.exports.checkDeviceConnected()
                    ...
                    .then(function () {
                        ...
                        var appFileInflated = path.join(buildOutputDir, 'Payload', projectName + '.deviceReady');
                        var appFile = path.join(buildOutputDir, projectName + '.deviceReady');
                        var payloadFolder = path.join(buildOutputDir, 'Payload');

                        // delete the existing platform/ios/build/device/appname.deviceReady
                        return spawn('rm', [ '-rf', appFile ], buildOutputDir)
                            .then(function () {
                                // move the platform/ios/build/device/Payload/appname.deviceReady to parent
                    ...
                    .then(function () {
                        appPath = path.join(projectPath, 'build', 'device', projectName + '.deviceReady');
                        var extraArgs = [];
                        if (runOptions.argv) {
...
/**
 * Deploy specified deviceReady package to connected device
...
/**
 * Deploy specified deviceReady package to ios-sim simulator
 * @param  {String} appPath Path to application package
