...
class QRScanner : CDVPlugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    // MARK: - 处理摄像头后者手电桶调用不了的问题
    private func handleError (error: QRScannerError) {
        if (error == .camera_access_denied ||
            error == .camera_unavailable ||
            error == .camera_access_restricted ||
            error == .light_unavailable) {
            alert(title: "Alert", message: "Camera permission denied. Please turn on the camera service") {
                UIApplication.shared.openURL(URL.init(string: UIApplicationOpenSettingsURLString)!)
            }
        }
    }
    func alert(title: String, message: String, okHandler: @escaping () -> ()) {
        let alertController = UIAlertController.init(title: title, message: message, preferredStyle: .alert)
        let cancelAction = UIAlertAction.init(title: "Cancel", style: .cancel, handler: nil)
        let ensureAction = UIAlertAction.init(title: "OK", style: .default) {(action) in
            okHandler()
        }
