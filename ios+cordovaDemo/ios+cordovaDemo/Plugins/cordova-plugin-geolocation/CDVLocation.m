...
{
    [self handleLocationError:errorCode];
    NSMutableDictionary* posError = [NSMutableDictionary dictionaryWithCapacity:2];
...
#pragma mark - 处理定位失败（添加）
- (void)handleLocationError:(CDVLocationStatus)code{
    switch (code) {
        case PERMISSIONDENIED: // 没有权限，拒绝访问
            [self alertWithTittle:@"Alert" message:@"Position permission denied. Please turn on location service" okHandler:^{
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];
            }];
            break;
        case POSITIONUNAVAILABLE: // 位置信息获取不到
            [self toast:@"position unavailable"];
            break;
        case TIMEOUT: // 定位超时
            [self toast:@"location time out"];
            break;
        default:
            break;
    }
...
- (void) toast: (NSString *)message {
    dispatch_async(dispatch_get_main_queue(), ^{
        [[UIApplication sharedApplication].delegate.window.rootViewController.view makeToast:message];
    });
}
// alert
- (void) alertWithTittle:(NSString *)title message:(NSString *)message okHandler:(void(^)(void))okHandler {
